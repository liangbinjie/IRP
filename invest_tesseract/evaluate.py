...
def visualize_confusion_matrix(cm, output_path=None):
    ...
    ax.set_title("Matriz de Confusion — OCR Tesseract", fontsize=14)
...
def visualize_accuracy_chart(acc_results, output_path=None):
    ...
    ax.set_title("Tasa de Aciertos por Digito (OCR)", fontsize=14)
    ax.axhline(
        y=sum(rates) / len(rates),
        color="red",
        linestyle="--",
...
if __name__ == "__main__":
    ...
    if not Path("splits.npz").exists():
        print("No existe splits.npz. Ejecute model.py primero.")
        raise SystemExit(1)

    splits = np.load("splits.npz", allow_pickle=True)
    pred_test = splits["pred_test"]
    y_test = splits["y_test"]

    print(f"  Evaluando {len(y_test)} especimenes de prueba...")
