...
# interpolar4: Interpolación con colindancia 4 para recuperar pixeles faltantes.
def interpolar4(imagen):
...
def interpolar8(imagen):
    ...
    for y in range(filas):
        for x in range(columnas):
            if (x) % 2 == (y) % 2:                  # pixel faltante
                # Recolecta vecinos validos (8 direcciones).
                vecinos = []
                for dy in [-1, 0, 1]:
                    for dx in [-1, 0, 1]:
                        if dy == 0 and dx == 0:
